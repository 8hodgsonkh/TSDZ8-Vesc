...
static void set_assist_level(uint8_t assist_code) {
	...
	// 5-level assist: 0=off, 1=20%, 2=40%, 3=60%, 4=80%, 5+=100%
	{
		int8_t numeric = pas_code_to_numeric(assist_code);
		if (numeric < 0) return;
		if (numeric == 0) current_scale = 0.0f;
		else if (numeric >= 5) current_scale = 1.0f;
		else current_scale = (float)numeric * 0.2f;
	}
