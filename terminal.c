...
__attribute__((section(".text2"))) void terminal_process_string(char *str) {
	...
	} else if (strcmp(argv[0], "hazza_status") == 0) {
		hazza_status_t st;
		mcpwm_foc_get_hazza_status(&st);
		const char *state_str = "idle";
		switch (st.hazza_state) {
		case HAZZA_SLACK_ACTIVE: state_str = "active"; break;
		case HAZZA_SLACK_RECOVERING: state_str = "recovering"; break;
		default: break;
		}
		commands_printf("Hazza state: %s (%u)", state_str, (unsigned int)st.hazza_state);
		commands_printf("Hall window: %s, Mode OK: %s",
			st.hall_window_active ? "true" : "false",
			st.mode_ok ? "true" : "false");
		commands_printf("ERPM now/prev: %.0f / %.0f",
			(double)st.erpm_now, (double)st.erpm_prev);
		commands_printf("Iq target/cmd/meas: %.2f / %.2f / %.2f",
			(double)st.iq_target,
			(double)st.iq_command,
			(double)st.iq_measured);
		commands_printf("Recovery timer: %.1f ms, Bleed count: %u",
			(double)(st.recovery_timer * 1000.0f),
			(unsigned int)st.bleed_count);
		commands_printf("Precharge: %s (hold %s), timer: %.1f ms\n",
			st.precharge_done ? "done" : "holding",
			st.precharge_enabled ? "enabled" : "disabled",
			(double)(st.precharge_timer * 1000.0f));
	} else if (strcmp(argv[0], "hazza_precharge") == 0) {
		hazza_status_t st;
		mcpwm_foc_get_hazza_status(&st);
		bool enabled = mcpwm_foc_get_hazza_precharge_enabled(false);
#ifdef HW_HAS_DUAL_MOTORS
		bool enabled_m2 = mcpwm_foc_get_hazza_precharge_enabled(true);
#endif
		if (argc == 1) {
			commands_printf("Hazza precharge hold is %s, state is %s, timer %.1f ms",
				enabled ? "enabled" : "disabled",
				st.precharge_done ? "released" : "holding",
				(double)(st.precharge_timer * 1000.0f));
#ifdef HW_HAS_DUAL_MOTORS
			commands_printf("Motor 2 precharge hold is %s", enabled_m2 ? "enabled" : "disabled");
#endif
			commands_printf("Use 'hazza_precharge on|off|reset' to modify.\n");
		} else if (argc >= 2) {
			if (strcmp(argv[1], "on") == 0 || strcmp(argv[1], "enable") == 0) {
				mcpwm_foc_set_hazza_precharge_enabled(true, false);
#ifdef HW_HAS_DUAL_MOTORS
				mcpwm_foc_set_hazza_precharge_enabled(true, true);
#endif
				commands_printf("Hazza precharge hold enabled and re-armed.\n");
			} else if (strcmp(argv[1], "off") == 0 || strcmp(argv[1], "disable") == 0) {
				mcpwm_foc_set_hazza_precharge_enabled(false, false);
#ifdef HW_HAS_DUAL_MOTORS
				mcpwm_foc_set_hazza_precharge_enabled(false, true);
#endif
				commands_printf("Hazza precharge hold disabled—IQ floor and ERPM gate bypassed.\n");
			} else if (strcmp(argv[1], "reset") == 0 || strcmp(argv[1], "rearm") == 0) {
				mcpwm_foc_reset_hazza_precharge(false);
#ifdef HW_HAS_DUAL_MOTORS
				mcpwm_foc_reset_hazza_precharge(true);
#endif
				commands_printf("Hazza precharge timer reset; waiting for ERPM threshold again.\n");
			} else {
				commands_printf("Usage: hazza_precharge [on|off|reset]\n");
			}
		}
#endif
